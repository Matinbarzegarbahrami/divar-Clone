    ...
    <>
      <nav className="lg:flex lg:flex-col grid grid-cols-3 gap-2 text-zinc-300/55 pr-5">
        ...
        {CATEGORY_SEARCH_LIST.map((item) => {
          ...
          return (
            <Link href={`/${item.name}?city=${city? city:"tehran"}`} key={item.id} className={`flex lg:justify-start justify-center gap-1.5 items-center lg:gap-4 ${slug ? item.name == slug ? 'text-zinc-50' : null : null}`}>
