...
export default function Post({ post }: { post: State }) {
  ...
  return (
    <Link
      ...
      key={post.id}
      className="flex gap-4 border h-40 border-zinc-700 rounded-xl p-1 sm:p-3 bg-zinc-900/30 hover:bg-zinc-900/50 transition"
    >
      <div className="relative w-28 sm:w-36 h-full flex-shrink-0">
        <Image
          src={imageSrc}
          alt={post.title}
          fill
      ...
      <div className="flex flex-col justify-between flex-1 min-w-0">
        <div>
          <h2 className="text-base font-bold truncate text-white line-clamp-2">
            {post.title}
          </h2>

          <p className="text-sm truncate text-zinc-400 mt-1">
